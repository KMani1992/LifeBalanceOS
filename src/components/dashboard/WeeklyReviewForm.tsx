...
/**
 * Returns the ISO date (YYYY-MM-DD) for the Monday of the current week in UTC.
 */
...
function getCurrentWeekStartIso(): string {
  ...
  utcNow.setUTCDate(utcNow.getUTCDate() + diffToMonday);
...
export default function WeeklyReviewForm({
  ...
  // Generated by GitHub Copilot - Auto-generate weekly summary with AI suggestions
  useEffect(() => {
    if (hasUserEditedNotes || !weekStart) {
      return;
    }
    ...
    async function generateSummary() {
      try {
        ...
        const weekTasks = tasks.filter((task) => {
          const resolvedDate = task.taskDate ?? task.createdAt.slice(0, 10);
          const taskDate = new Date(`${resolvedDate}T00:00:00.000Z`);
          return taskDate >= weekStartDate && taskDate <= weekEndDate;
        });

        // Generate task summary
        const summary = generateTaskSummary(weekTasks);

        // Fetch AI suggestion (will use fallback if API fails)
        const { suggestion, error } = await fetchAISuggestion(summary, {
          career: careerScore,
          family: familyScore,
        ...
        // Format and set notes only if user hasn't edited
        const formattedNotes = formatSummaryAsText(summary, suggestion);
        setNotes(formattedNotes);

        // Generated by GitHub Copilot - Persist AI suggestion history for trend review
        if (user && suggestion.trim()) {
          saveWeeklySuggestionHistory({
            ...
            lifeBalanceScore: Math.round((careerScore + familyScore + financeScore + peaceScore) / 4),
          });
        }

        if (error) {
          setSummaryError(`Note: AI suggestions temporarily unavailable. Fallback summary provided.`);
        }
      } catch (err) {
        if (active) {
  ...
  }, [weekStart, tasks, careerScore, familyScore, financeScore, peaceScore, hasUserEditedNotes, user]);
  ...
  const suggestedScores = useMemo(() => {
    ...
    tasks.filter((task) => inWeek(task.taskDate ?? task.createdAt)).forEach((task) => {
          ...
          >
            ...
            <Grid item xs={12} sx={{ minWidth: 0, px: { xs: 0.25, sm: 0 } }}>
              <Stack spacing={1}>
                ...
                <TextField
                  label="Weekly notes with AI suggestions"
                  value={notes}
                  onChange={(event) => {
                  ...
                  multiline
                  minRows={3}
                  placeholder="Auto-filled with your task summary and AI suggestions. Edit as needed."
                  helperText={
                    isGeneratingSummary
